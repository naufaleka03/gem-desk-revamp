...
<div>
    <div class="row">
        <div class="col-lg-12 margin-tb d-flex align-items-center"
            ...
            <h2 style="padding-bottom: 10px; margin-bottom: 10px;">{{ isset($user) ? 'Edit User' : 'Add New User' }}</h2>
        </div>
    </div>
    ...
    <form action="{{ isset($user) ? route('userManagements.update', $user->id) : route('userManagements.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        @if(isset($user))
            @method('PATCH')
        @endif
        ...
        <div class="card mb-4">
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <input type="password" name="password" class="form-control" placeholder="{{ isset($user) ? 'Leave blank to keep current password' : 'Password' }}">
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="organization_id" id="organization_select" class="form-control">
                        ...
                        @foreach ($organizations as $organization)
                            <option value="{{ $organization->id }}" @if(isset($user) && $user->organization_id == $organization->id) selected @endif>
                ...
                <div class="mb-3">
                    ...
                    <select name="roles" class="form-control">
                        <option value="admin" @if(isset($user) && $user->roles == 'admin') selected @endif>Admin</option>
                        <option value="user" @if(isset($user) && $user->roles == 'user') selected @endif>User</option>
                    </select>
                </div>
